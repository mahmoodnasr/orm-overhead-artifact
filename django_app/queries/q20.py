...
from django.db.models import Sum, F, Q, Subquery, OuterRef, DecimalField, ExpressionWrapper
...
def run_query_orm(using='default', params=None):
    ...
    forest_parts = Part.objects.using(using).filter(
        name__startswith=P['color']
    ...
    lineitem_threshold = (
        ...
        .annotate(half_qty=ExpressionWrapper(
            Sum('quantity') * Decimal('0.5'),
    ...
    eligible_suppliers = (
        ...
        .using(using)
        .filter(
            partkey__in=forest_parts,
            availqty__gt=Subquery(lineitem_threshold)
        )
    ...
    results = (
        ...
        .filter(
            suppkey__in=eligible_suppliers,
            nationkey__name=P['nation']
        )
        .annotate(
            s_name=F('name'),
            s_address=F('address')
