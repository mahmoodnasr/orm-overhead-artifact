...
Very Complex query with 7 table joins

Finds the market share of a specific nation for a specific part type
...
def run_query_orm(using='default', params=None):
    ...
    results = (
        LineItem.objects
        .using(using)
        .select_related('partkey', 'suppkey__nationkey', 'orderkey__custkey__nationkey__regionkey')
        .filter(
        ...
        .annotate(
            mkt_share=F('brazil_volume') / F('total_volume')
        )
        .order_by('o_year')
