...
def run_query_orm(using='default', params=None):
    ...
    results = (
        ...
        .values(
            'orderkey',
            'orderkey__orderdate',
            'orderkey__shippriority'
        )
        .annotate(
            revenue=Sum(F('extendedprice') * (1 - F('discount')))
        )
...
def run_query_sql(connection, params=None):
    ...
    if vendor == 'mysql':
        ...
        limit_clause = "OFFSET 0 ROWS FETCH NEXT 10 ROWS ONLY"
    else:  # PostgreSQL, Oracle
        limit_clause = "FETCH FIRST 10 ROWS ONLY"
