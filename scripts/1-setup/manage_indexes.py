...
try:
    ...
    if not hasattr(oracledb, "IntegrityError"):
        oracledb.IntegrityError = (
            oracledb.IntegrityError
            if hasattr(oracledb, "IntegrityError")
            else oracledb.DatabaseError
        )
...
def get_db_vendor(connection):
    ...
    if "postgres" in vendor:
        ...
        return "oracle"
    elif "microsoft" in vendor or "sqlserver" in str(
...
def create_indexes(db_alias):
    ...
    if INDEXES is TPCC_INDEXES:
        ...
        index_columns = {
            ...
            "idx_lineitem_shipdate_discount_qty": (
                "l_shipdate",
                "l_discount",
                "l_quantity",
            ),
    ...
    print(f"\n{'=' * 70}")
    print(
        f"Summary: {created_count} created, {skipped_count} skipped, {failed_count} failed"
    )
    print(f"{'=' * 70}")
...
def drop_indexes(db_alias):
    ...
    print(f"\n{'=' * 70}")
    print(
        f"Summary: {dropped_count} dropped, {skipped_count} skipped, {failed_count} failed"
    )
    print(f"{'=' * 70}")
...
def verify_indexes(db_alias, expect):
    ...
    print(
        f"index state on {db_alias} ({vendor}): "
    ...
    if ok:
        ...
        print(
            "  DOES NOT MATCH - refusing to let a run be filed under the "
            "wrong configuration."
        )
...
Examples:
  ...
  python manage_indexes.py --all --action drop
    ...
    parser.add_argument(
        "--all", "-a", action="store_true", help="Operate on all databases"
    )
    ...
    if args.benchmark == "tpcc":
        ...
        print(
            f"managing the TPC-C index set ({sum(len(v) for v in INDEXES.values())} "
