...
BLOCKED_QUERY = {
    ...
    **{
        (
            ...
            "indexed",
            q,
        ): "not_run: LINEITEM's six secondary indexes are ~1.1 GB "
...
FIELDS = [
    ...
    "orm_median_qerror",
    "orm_max_qerror",
    "orm_operators_qerror_gt_10",
    "orm_underestimate_rate_pct",
    "sql_plan_operators",
    "sql_median_qerror",
    "sql_max_qerror",
    "sql_operators_qerror_gt_10",
    "sql_underestimate_rate_pct",
    "orm_plan_file",
    "sql_plan_file",
    # implementation validation
    "validated_orm_path_is_orm",
    "validated_results_match",
    "validated_baselines_match",
    "validated_orm_equals_sql",
...
def load_measurements(root, scale="10"):
    ...
    for dirpath, _dirs, files in ((root, [], sorted(os.listdir(root))),):
        for fn in sorted(f for f in files if os.path.isfile(os.path.join(root, f))):
...
def _ceiling_words(raw):
    ...
    v = (
        float(m.group(1))
        * {"ms": 0.001, "s": 1.0, "min": 60.0, "h": 3600.0, None: 0.001}[m.group(2)]
    )
    return "%g s" % v
            ...
            base.update(
                {
                    ...
                    "note": (
                        "censored: exceeded the %s ceiling in warmup, "
                        "recorded as a one-sided bound (section 6)"
                        % _ceiling_words(cr.get("ceiling_s", ""))
                        if st == "timeout"
                        else "not expressible in this ORM on this vendor (C17)"
                    ),
                    "paired_ratio": "",
                }
            )
    ...
    for rel in files:
        ...
        if dbkey == "oracle" and schema is None:
            ...
            sib = (
                os.path.join(
                    root,
                    os.path.dirname(rel),
                    "oracle_group_%s.measure.log" % m.group(1),
                )
                if m
                else ""
            )
            if os.path.exists(sib):
                mm = re.search(
                    r"=== Q\d\d \([^)]*\)\s+oracle\s+(non-indexed|indexed)",
        ...
        for line in lines:
            ...
            if schema is None:
                continue
            m = re.match(
                r"^(q\d\d)\s+(yes|NO|no|n/a)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+\d", s
            )
            if m:
...
def main():
    ...
    ap.add_argument(
        "--scale",
        default=os.environ.get("TPCH_SF", "10"),
    ...
    ap.add_argument(
        "--qerror", default=None, help="directory tree searched for qerror_by_query.csv"
    )
        ...
        for schema, schema_label in SCHEMAS:
            for bench, qid, qname, band in WORKLOAD:
                for fw, fw_label, fwver in ORMS:
                    ...
                    sql_t = (
                        f(sql, "median_s")
                        if sql and sql.get("status") == "ok"
                        else None
                    )
                    orm_t = (
                        f(orm, "median_s")
                        if orm and orm.get("status") == "ok"
                        else None
                    )
                    ...
                    if sql_t and orm_t:
                        ...
                        pr = (orm or {}).get("paired_ratio") or (sql or {}).get(
                            "paired_ratio"
                        )
                    ...
                    if sql is None and orm is None:
                        ...
                        gate = (
                            val.get((dbkey, schema, qid)) if bench == "tpch" else None
                        )
                        if gate and any(
                            x not in ("pass", "not_recorded") for x in gate[:5]
                        ):
                            note = (
                                "not_run: gated by validation (C23) - ORM?=%s MATCH=%s "
                                "SQL==%s ORM=SQL=%s ROWS>0=%s [%s]" % gate
                            )
                        else:
                            note = BLOCKED_QUERY.get(
                                (dbkey, schema, qid)
                            ) or BLOCKED.get((dbkey, bench), "not_run")
                    else:
                        note = " | ".join(
                            x.get("note", "") for x in (sql, orm) if x and x.get("note")
                        )
                    ...
                    xm = CROSS_MACHINE_QUERY.get((dbkey, qid)) or CROSS_MACHINE.get(
                        dbkey
                    )
                    ...
                    vv = (
                        val.get((dbkey, schema, qid), NOT_RECORDED)
                        if bench == "tpch"
                        else NOT_RECORDED
                    )
                    if (
                        (sql_t is not None or orm_t is not None)
                        and bench == "tpch"
                        ...
                        if "NotExpressible" in vv[0]:
                            ...
                            c23 = (
                                "checks never ran (C23): the validator stops at "
                                "Django's Q13NotExpressible (C17) before comparing "
                                "this path [%s]" % vv[5]
                            )
                        else:
                            c23 = (
                                "measured without a passing validation (C23): "
                    ...
                    row.update(
                        {
                            ...
                            "orm_version": NOT_YET_RECORDED
                            if not legacy
                            else fwver
                            + (
                                UNLOGGED_ORM_QUALIFIER
                            ...
                            "dbms_version": (
                                (
                                    DBMS_VERSION_QUERY.get((dbkey, schema, qid))
                            ...
                            "rows_returned": (orm or sql or {}).get(
                                "rows_returned", ""
                            ),
                            "sql_min_s": f(sql, "min_s") or "",
                            "sql_max_s": f(sql, "max_s") or "",
                            "sql_cv_pct": f(sql, "cv_pct") if sql else "",
                            "orm_min_s": f(orm, "min_s") or "",
                            "orm_max_s": f(orm, "max_s") or "",
                            "orm_cv_pct": f(orm, "cv_pct") if orm else "",
                            "repetitions_measured": (orm or sql or {}).get(
                                "repetitions_measured", ""
                            ),
                            "sql_status": (
                                "invalid"
                                if bad and sql
                                else (sql.get("status") if sql else "not_run")
                            ),
                            "orm_status": (
                                "invalid"
                                if bad and orm
                                else (orm.get("status") if orm else "not_run")
                            ),
                            ...
                            "orm_operators_qerror_gt_10": qo.get(
                                "operators_qerror_gt_10", ""
                            ),
                            "orm_underestimate_rate_pct": qo.get(
                            ...
                            "sql_operators_qerror_gt_10": qsq.get(
                                "operators_qerror_gt_10", ""
                            ),
                            "sql_underestimate_rate_pct": qsq.get(
                                "underestimate_rate_pct", ""
                            ),
    ...
    notrun = [
        r for r in rows if r["sql_status"] == "not_run" and r["orm_status"] == "not_run"
    ]
    ...
    state = next(
        (
            c
            for c in (
                os.path.join(args.measurements, ".campaign_state"),
                os.path.join(
                    os.path.dirname(args.measurements.rstrip("/")), ".campaign_state"
                ),
    ...
    for _, dblabel, _ in DBMS:
        for bench in ("tpch", "tpcc"):
            for _, schema_label in SCHEMAS:
                ...
                print(
                    f"  {dblabel:12s} {bench:6s} {schema_label:12s} "
    ...
    for _, dblabel, _ in DBMS:
        for _, schema_label in SCHEMAS:
            for _, fwl, _ in ORMS:
                v = [
                    float(r["overhead_percentage"])
                    for r in rows
                    if r["dbms"] == dblabel
                    and r["schema_config"] == schema_label
                    and r["orm"] == fwl
                    and r["overhead_percentage"] != ""
                ]
                if v:
                    print(
                        f"  {dblabel:12s} {fwl:11s} {schema_label:12s} "
...
WORK_ORDER = [
    (
        "PostgreSQL",
        "TPC-H",
        "re-measured at 8 parallel workers; the 2-worker "
        "sandbox rows are archived under measurements/superseded/",
    ),
    (
        "MySQL",
        "TPC-H",
        "done - single-threaded per query, which no setting "
        "changes (docs/PARALLELISM.md)",
    ),
    (
        "SQL Server",
        "TPC-H",
        "done - MAXDOP 8, under x86-64 emulation; Q13's "
        "Django ORM cell is inexpressible (C17)",
    ),
    (
        "Oracle",
        "TPC-H",
        "done as far as the licence allows - 12 GB cap, run per "
...
def remaining_markdown(rows):
    ...
    note = (
        "done - latency in this grid, QPM at concurrency 50 in "
        "measurements/tpcc_throughput.csv (Oracle QPM outstanding)"
        if tpcc_left == 0
        else "in progress - %d of %d cells still to measure" % (tpcc_left, len(tpcc))
    )
...
def status_markdown(rows, state_path=None):
    ...
    out.append("")
    out.append(
        "## Status: %d of %d rows emitted, %d measured (%.0f%%), %d with q-error"
        ...
        for bench in ("tpch", "tpcc"):
            for _, schema_label in SCHEMAS:
                ...
                out.append(
                    ...
                    % (
                        dblabel,
                        bench.upper().replace("TPCH", "TPC-H").replace("TPCC", "TPC-C"),
                        schema_label,
                        b.count("complete"),
                        b.count("partial"),
                        b.count("invalid"),
    ...
    for _, dblabel, _ in DBMS:
        for _, schema_label in SCHEMAS:
            ...
            for _, fwl, _ in ORMS:
                ...
                cells.append(
                    "%+.1f%% (n=%d)" % (statistics.median(v), len(v)) if v else "—"
                )
