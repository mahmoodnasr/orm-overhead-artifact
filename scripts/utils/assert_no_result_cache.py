...
def check_oracle():
    ...
    cn = oracledb.connect(
        user=os.environ.get("ORA_SYS_USER", "system"),
    ...
    return (
        ...
        detail,
        (
            "MANUAL is Oracle's default and means the cache is used "
...
def check_mysql():
    ...
    return (
        ok,
        "MySQL %s, have_query_cache: %s" % (ver, present or "absent"),
        (
...
def check_postgresql():
    ...
    dsn = os.environ.get(
        "PG_DSN", "host=127.0.0.1 port=55432 dbname=tpch user=benchmark password=bench"
    )
    ...
    return (
        True,
        "PostgreSQL %s, effective_cache_size = %s (a planner hint)" % (ver, ecs),
        (
...
def main():
    ...
    try:
        ...
        print(
            "  result-cache check skipped for %s: %s" % (args.dbms, str(e)[:100]),
            file=sys.stderr,
        )
        return 0
    ...
    if not ok:
        print(
            "REFUSING TO MEASURE: results would be cache lookups, not query "
            "executions.",
            file=sys.stderr,
        )
        return 1
