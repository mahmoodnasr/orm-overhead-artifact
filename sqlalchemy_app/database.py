...
def get_connection_string(database: str) -> str:
    ...
    if database not in DB_CONFIGS:
        raise ValueError(f"Unknown database: {database}. Available: {list(DB_CONFIGS.keys())}")
    ...
    def get_engine(self, database: str, echo: bool = False) -> Engine:
        ...
        if database not in self._engines:
            ...
            if database == 'oracle':
                ...
                import types
                original_preparer.quote_identifier = types.MethodType(uppercase_quote, original_preparer)
