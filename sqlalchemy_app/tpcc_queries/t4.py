...
def run_transaction_orm(session: Session):
    ...
    try:
        ...
        for d_id in range(1, DISTRICTS_PER_WAREHOUSE + 1):
            new_order = session.query(NewOrder).filter_by(
                no_w_id=w_id, no_d_id=d_id
            ).order_by(NewOrder.no_o_id).first()

            if not new_order:
                continue

            o_id = new_order.no_o_id
            order = session.query(Order).filter_by(o_w_id=w_id, o_d_id=d_id, o_id=o_id).first()
        ...
        session.commit()
        return {'w_id': w_id, 'carrier_id': carrier_id, 'orders_delivered': len(results)}
    except Exception:
        session.rollback()
        raise
