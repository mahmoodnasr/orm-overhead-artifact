...
def run_transaction_orm(session: Session):
    ...
    from sqlalchemy_app.tpcc_models import Warehouse, District, Customer, Item, Stock, Order, NewOrder, OrderLine
    ...
    try:
        ...
        session.execute(
            text("UPDATE district SET d_next_o_id = :new_id WHERE d_w_id = :w_id AND d_id = :d_id"),
            {'new_id': next_o_id + 1, 'w_id': w_id, 'd_id': d_id}
        )
        session.flush()

        order = Order(
            o_id=next_o_id, o_d_id=d_id, o_w_id=w_id,
            o_c_id=c_id, o_entry_d=datetime.now(), o_carrier_id=None,
        ...
        for ol_number in range(1, ol_cnt + 1):
            ...
            order_line = OrderLine(
                ol_o_id=next_o_id, ol_d_id=d_id, ol_w_id=w_id, ol_number=ol_number,
                ol_i_id=ol_i_id, ol_supply_w_id=w_id, ol_quantity=ol_quantity,
                ol_amount=ol_amount, ol_dist_info=stock.s_dist_01
            )
            session.add(order_line)

        session.commit()
        return {'w_id': w_id, 'd_id': d_id, 'c_id': c_id, 'o_id': next_o_id, 'total_amount': total_amount}
    except Exception:
        session.rollback()
        raise
...
def run_transaction_sql(session: Session):
    ...
    try:
        ...
        """), {'w_id': w_id}).first()
        if row is None:
            raise LookupError(f"warehouse {w_id} not found; tpcc_config does not "
                              f"describe the loaded database")

        row = session.execute(text("""
            SELECT c_discount FROM customer
            WHERE c_w_id = :w_id AND c_d_id = :d_id AND c_id = :c_id
        """), {'w_id': w_id, 'd_id': d_id, 'c_id': c_id}).first()
        if row is None:
        ...
        """), {
            'o_id': next_o_id, 'd_id': d_id, 'w_id': w_id, 'c_id': c_id, 'ol_cnt': ol_cnt
        })
        ...
        for ol_number in range(1, ol_cnt + 1):
            ...
            item_row = session.execute(
                text("SELECT i_price FROM item WHERE i_id = :i_id"),
                {'i_id': ol_i_id}).first()
            if item_row is None:
            ...
            """), {'o_id': next_o_id, 'd_id': d_id, 'w_id': w_id,
                   'ol_number': ol_number, 'i_id': ol_i_id, 'supply_w_id': w_id,
                   'qty': ol_quantity, 'amount': ol_amount,
                   'dist_info': stock_row[0]})

        session.commit()
        return {'w_id': w_id, 'd_id': d_id, 'c_id': c_id, 'o_id': next_o_id,
                'ol_cnt': ol_cnt, 'total_amount': total_amount}
    except Exception:
        session.rollback()
        raise
