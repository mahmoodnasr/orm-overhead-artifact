...
def q01(session: Session, p):
    rows = (
        session.query(
            ...
            func.sum(LineItem.extendedprice * (1 - LineItem.discount)).label(
                "sum_disc_price"
            ),
            func.sum(
                LineItem.extendedprice * (1 - LineItem.discount) * (1 + LineItem.tax)
            ).label("sum_charge"),
...
def q02(session: Session, p):
    ...
    rows = (
        ...
        .filter(
            Part.partkey == PartSupp.partkey_id,
...
def q03(session: Session, p):
    rows = (
        ...
        .order_by(
            func.sum(LineItem.extendedprice * (1 - LineItem.discount)).desc(),
            Orders.orderdate,
        )
        .limit(10)
        .all()
...
def q04(session: Session, p):
    ...
    rows = (
        ...
        .filter(
            Orders.orderdate >= p["date"], Orders.orderdate < p["date_end"], exists_q
        )
...
def q06(session: Session, p):
    row = (
        session.query(
            func.sum(LineItem.extendedprice * LineItem.discount).label("revenue")
        )
        .filter(
            LineItem.shipdate >= p["date"],
...
def q07(session: Session, p):
    ...
    rows = (
        session.query(
            n1.name.label("supp_nation"),
            n2.name.label("cust_nation"),
            year,
            func.sum(volume).label("revenue"),
        )
        .filter(
            ...
            or_(
                and_(n1.name == p["nation1"], n2.name == p["nation2"]),
...
def q09(session: Session, p):
    ...
    rows = (
        session.query(
            Nation.name.label("nation"), year, func.sum(amount).label("sum_profit")
        )
        .filter(
            Supplier.suppkey == LineItem.suppkey_id,
...
def q11(session: Session, p):
    ...
    threshold = (
        ...
        select(func.sum(ps2.supplycost * ps2.availqty) * Decimal(repr(Q11_FRACTION)))
        .where(
            and_(
                ps2.suppkey_id == s2.suppkey,
                s2.nationkey_id == n2.nationkey,
                n2.name == p["nation"],
            )
        )
    ...
    rows = (
        ...
        .filter(
            PartSupp.suppkey_id == Supplier.suppkey,
            Supplier.nationkey_id == Nation.nationkey,
...
def q12(session: Session, p):
    high = func.sum(
        case(
            (
                or_(
                    Orders.orderpriority == "1-URGENT", Orders.orderpriority == "2-HIGH"
                ),
                1,
            ),
            else_=0,
...
def q13(session: Session, p):
    inner = (
        session.query(
            Customer.custkey.label("c_custkey"),
...
def q14(session: Session, p):
    ...
    row = (
        ...
        .filter(
            LineItem.partkey_id == Part.partkey,
            LineItem.shipdate >= p["date"],
            LineItem.shipdate < p["date_end"],
        )
        .one()
...
def q15(session: Session, p):
    rev = (
        session.query(
            LineItem.suppkey_id.label("supplier_no"),
            func.sum(LineItem.extendedprice * (1 - LineItem.discount)).label(
                "total_revenue"
            ),
        )
        .filter(LineItem.shipdate >= p["date"], LineItem.shipdate < p["date_end"])
    ...
    rows = (
        ...
        .filter(Supplier.suppkey == rev.c.supplier_no, rev.c.total_revenue == max_rev)
        .order_by(Supplier.suppkey)
        .all()
    )
    return _dicts(
        rows, ["s_suppkey", "s_name", "s_address", "s_phone", "total_revenue"]
    )
...
def q18(session: Session, p):
    ...
    rows = (
        ...
        .filter(
            Orders.orderkey.in_(big_orders),
            Customer.custkey == Orders.custkey_id,
            Orders.orderkey == LineItem.orderkey_id,
        )
        .group_by(
...
def q20(session: Session, p):
    forest_parts = (
        select(Part.partkey)
        .where(Part.name.like("%s%%" % p["color"]))
        .scalar_subquery()
    )
    half_qty = (
        ...
        .where(
            and_(
                LineItem.partkey_id == PartSupp.partkey_id,
    ...
    good_supp = (
        ...
        .where(
            and_(PartSupp.partkey_id.in_(forest_parts), PartSupp.availqty > half_qty)
        )
        .scalar_subquery()
    )
    rows = (
        ...
        .filter(
            Supplier.suppkey.in_(good_supp),
            Supplier.nationkey_id == Nation.nationkey,
            Nation.name == p["nation"],
        )
...
def q21(session: Session, p):
    ...
    exists_other = (
        select(literal_column("1"))
        .where(and_(l2.orderkey_id == l1.orderkey_id, l2.suppkey_id != l1.suppkey_id))
        .exists()
    )
    not_exists_late = (
        select(literal_column("1"))
    ...
    rows = (
        ...
        .filter(
            Supplier.suppkey == l1.suppkey_id,
...
def q22(session: Session, p):
    ...
    avg_bal = (
        ...
        .where(and_(c2.acctbal > Decimal("0.00"), _substr(c2.phone, 1, 2).in_(codes)))
        .scalar_subquery()
    )
    no_orders = (
        select(literal_column("1"))
